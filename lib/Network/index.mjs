...
const Network = {
   ...
   */
  createNeuron: (value = 0, bias = 0, leftAxons = [], rightAxons = []) => ({
    value,
    bias,
    leftAxons,
    rightAxons
  ...
  createNetwork: function(
    ...
    const trainSample = trainSample => {
      // Validate trainSample
      Validator.validateTrainSample(trainSample)
      // Launching NN
      ...
      errorGradient[layers.length - 1] = layers[layers.length - 1].neurons.map(
        (n, i) => {
    ...
    const restart = () => {
      layers.forEach(l => {
        l.neurons.map(n => {
          n.value = Math.randomRange(-5, 5)
          n.bias = Math.randomRange(-5, 5)
          n.leftAxons.forEach(
            axon => (axon.weight.value = Math.randomRange(-5, 5))
          )
        })
      })
      generation = 0
