...
const Network = {
   ...
   */
  connectLayers: (lx, ly, onlyWeight = Math.randomRange(-10, 10)) => {
    ly.neurons.map(ny =>
      lx.neurons.map(nx => Network.connectNeurons(ny, nx, onlyWeight))
    )
  },
  /*
  ...
  createNetwork: function(
      ...
      ...Array.from(Array(hidLayersCount), () =>
        Network.createLayer(
          Array.from(Array(hidLayersSize), () =>
            Network.createNeuron(Math.random(), Math.randomRange(-10, 10))
          )
        )
      ),
      Network.createLayer(
    ...
    const netInput = n =>
      n.leftAxons.reduce((acc, nl) => acc + nl.weight * nl.n.value, n.bias)
        ...
        for (let l = layers.length - 2; l >= 0; l--) {
          errorGradient[l] = layers[l].neurons.map(n => {
            ...
            const errToAct = n.rightAxons.reduce((acc, axon, aI) => {
              ...
              return acc + dAct * dOut * axon.weight
            }, 0)
            const outToNet = Math.dSigmoid(netInput(n))
            return {
              dAct: errToAct,
        ...
        for (let L = 1; L < layers.length; L++)
          layers[L].neurons.forEach((n, N) => {
            ...
            n.leftAxons.forEach((axon, A) => {
              axon.weight -= errorGradient[L][N].dWeights[A].dWeight
            })
          })
        epoch++
      })
